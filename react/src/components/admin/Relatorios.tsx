...
const Relatorio: React.FC = () => {
  ...
  // Função auxiliar para descobrir qual assinatura tem o maior número de adesões
  const obterAssinaturaLider = () => {
    if (!dados || !dados.assinaturas || !dados.assinaturas.porTipo) return { nome: "Nenhuma", qtd: 0 };
      ...
      <div className="max-w-6xl mx-auto">
        ...
        <div className="text-center mb-8">
          ...
          <p className="uppercase tracking-widest text-sm opacity-80 mt-2">
            relatorios de análise de desempenho da floricultura período: {new Date(dados.dataInicio).toLocaleDateString("pt-BR")} até {new Date(dados.dataFim).toLocaleDateString("pt-BR")} 
          </p>
          <div className="flex justify-center gap-2 mt-6">
            {(["semana", "mes", "ano"] as Periodo[]).map((p) => (
              ...
              >
                {p === "semana" ? "Última Semana" : p === "mes" ? "Último Mês" : "Último Ano"}
              </button>
            ))}
          </div>
        </div>
          ...
          <div className="bg-white rounded-2xl p-6 shadow">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              ...
              <div className="bg-rosa-claro/30 rounded-xl p-5 flex flex-col items-center text-center">
                ...
                <p className="text-2xl font-bold mt-1 text-rosa-text">{dados.vendas.produtoMaisVendido}</p>
                <p className="text-sm opacity-75">{dados.vendas.quantidadeProdutoMaisVendido} unid.</p>
              </div>
              ...
              <div className="bg-rosa-claro/30 rounded-xl p-5 flex flex-col items-center text-center">
                ...
                <p className="text-2xl font-bold mt-1 text-rosa-text">{dados.vendas.categoriaMaisVendida}</p>
              ...
              <div className="bg-rosa-claro/30 rounded-xl p-5 flex flex-col items-center text-center">
                ...
                <p className="text-2xl font-bold mt-1 text-rosa-text capitalize">{assinaturaLider.nome.toLowerCase()}</p>
