...
export default function DetalhesPage() {
  ...
  const handleAdicionarAoCarrinho = async (produto: Produto) => {
    ...
    try {
      ...
      console.error("Erro ao adicionar produto ao carrinho no servidor:", error);
  ...
  */
  ...
  const handleAdicionarAoCarrinho = async (produto: Produto) => {
    ...
    if (!tokenAtual) {
      console.error("BLOQUEADO: Nenhum token foi encontrado! Redirecionando para login.");
      navigate("/cliente-login");
      return;
    }

    console.log("Token validado com sucesso localmente. Disparando requisição POST...");
    ...
    try {
      ...
      console.log("Sucesso no servidor! Resposta:", response.data);
      navigate("/carrinho");
    } catch (error) {
      console.error("Ops, caiu no CATCH! Houve um erro na requisição:");
      ...
      <div className="container mx-auto max-w-5xl bg-white rounded-2xl p-6 md:p-8 shadow-sm border border-slate-100">
        ...
        <section className="bg-white rounded-3xl shadow-xl p-4 md:p-6 border border-rosa-pastel">
          ...
          <form className="grid grid-cols-2 md:grid-cols-5 gap-6" onSubmit={handleSubmit}>
            <div className="grid gap-4 lg:grid-cols-1 col-span-4">
